...
from rent_control.models import ZoneTresTendue


class Command(BaseCommand):
    help = "Import zones très tendues (Zone A bis) from CSV/ODS file"

    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            type=str,
            default="algo/zone_tres_tendue/zone_tres_tendue.ods",
            help="Path to the CSV/ODS file relative to the project root",
        )
        parser.add_argument(
            "--batch-size",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if file_path.endswith(".ods"):
                ...
                raise CommandError(
                    f"Unsupported file format. Please use .csv or .ods file."
                )
            ...
            if clear_data:
                ...
                ZoneTresTendue.objects.all().delete()
        ...
        with transaction.atomic():
            for _, row in batch_df.iterrows():
                try:
                    ...
                    departement_code = self._clean_value(
                        row.get("DÉPARTEMENT CODE", "")
                    )
                    departement_nom = self._clean_value(row.get("DÉPARTEMENT", ""))
                    commune = self._clean_value(row.get("COMMUNE", ""))

                    if not departement_code or not departement_nom or not commune:
                        self.stdout.write(
                            self.style.WARNING(
                    ...
                    # Unique key: departement_code + commune
                    zone, created = ZoneTresTendue.objects.update_or_create(
